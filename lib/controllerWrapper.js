...
module.exports = function(fn){
    if (typeof fn == 'function'){
        ...
        if (fn.constructor.name == 'GeneratorFunction'){
            return controllerCoroutineWrapper(fn)
        }
    }
    return fn
}
...
/**
 * Wrap a user defined controller as a Coroutine
 * 
 * @param {GeneratorFunction} fn 
 * @returns {Function}
 */
function controllerCoroutineWrapper(fn){
    ...
    return function expressDeliverController(req,res,next){
        ...
        bluebird.coroutine(fn.bind(res.locals, req, res, err => {
 ...
 * Almost the same as Coroutine one, but calling fn directly
